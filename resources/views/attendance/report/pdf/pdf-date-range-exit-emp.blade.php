            ...
            <td style="width:90%">
                <table style="width:100%" class="order-bank">
                    ...
                    <tr>
                        ...
                        Early Exit Employee Report </span>: {!! $employees->count() !!}</td>
                    </tr>
...
@if($data->contains('department_id',$dept->department_id))
    ...
    <table class="table order-bank" width="90%" cellpadding="2">
        ...
        <tr class="row-line">
            ...
            <th width="60px" style="text-align: left; font-size: 10px; font-weight: bold">Shift Exit</th>
            <th width="50px" style="text-align: left; font-size: 10px; font-weight: bold">Exit Time</th>
            <th width="80px" style="text-align: left; font-size: 10px; font-weight: bold">Date</th>
        </tr>
        </thead>
        <tbody>
        @php($sl = 1)
        ...
        @foreach($data as $row)
            @if($dept->department_id == $row->department_id)
                ...
                <tr>
                    ...
                    <td width="145px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->professional->personal->full_name !!} </td>
                    <td width="125px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->professional->designation->name !!} </td>

                    <td width="60px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! \Carbon\Carbon::parse($row->shift_exit_time)->format('g:i A') !!}</td>
                    <td width="50px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! \Carbon\Carbon::parse($row->exit_time)->format('g:i A') !!}</td>
