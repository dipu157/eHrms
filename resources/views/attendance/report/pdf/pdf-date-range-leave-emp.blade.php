...
@if($data->contains('department_id',$dept->department_id))
    ...
    <table class="table order-bank" width="90%" cellpadding="2">
        ...
        <tr class="row-line">
            ...
            <th width="95px" style="text-align: left; font-size: 10px; font-weight: bold">Name</th>
            <th width="70px" style="text-align: center; font-size: 10px; font-weight: bold">Designation</th>
            <th width="55px" style="text-align: center; font-size: 10px; font-weight: bold">Joining Date</th>
            <th width="90px" style="text-align: center; font-size: 10px; font-weight: bold">Leave Name</th>
            <th width="90px" style="text-align: center; font-size: 10px; font-weight: bold">Shift</th>
        ...
        @foreach($data as $row)
            @if($dept->department_id == $row->department_id)
                ...
                <tr>
                    ...
                    <td width="95px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->professional->personal->full_name !!} </td>
                    <td width="70px" style="border-bottom-width:1px; font-size:8pt; text-align: center">{!! $row->professional->designation->name !!} </td>
                    ...
                    <td  width="90px" style="border-bottom-width:1px; font-size:8pt; text-align: center">{!! $row->leave->name !!}</td>
                    <td width="90px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!!$row->shift->short_name !!}--{!! \Carbon\Carbon::parse($row->shift_entry_time)->format('g:i A') !!}--{!! \Carbon\Carbon::parse($row->shift_exit_time)->format('g:i A') !!}</td>
                    <td width="70px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! \Carbon\Carbon::parse($row->attend_date)->format('d-M-Y') !!}</td>
                </tr>
                @php($sl++)
            @endif
        @endforeach
