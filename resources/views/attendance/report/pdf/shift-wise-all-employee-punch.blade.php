  ...
  @foreach($departments as $i=>$dept)
    ...
    @if($dptname->contains('employee_id',$dept->department->headed_by))
        ...
        <tr class="row-line">
            ...
            <th width="40px" style="text-align: left; font-size: 10px; font-weight: bold">In Time</th>
            <th width="40px" style="text-align: left; font-size: 10px; font-weight: bold">Exit Time</th>
...
@foreach($dptname as $row)
     @if($row->employee_id == $dept->department->headed_by)
        ...
        <tr>
            ...
            <td width="40px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! is_null($row->entry_time) ? '' : \Carbon\Carbon::parse($row->entry_time)->format('g:i A') !!}</td>
            <td width="40px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! is_null($row->exit_time) ? '' : \Carbon\Carbon::parse($row->exit_time)->format('g:i A') !!}</td>
...
@if($dptname->contains('shift_id',$spt->shift_id))
    ...
    <table class="table order-bank" width="90%" cellpadding="2">
        ...
        <tr class="row-line">
            ...
            <th width="35px" style="text-align: left; font-size: 10px; font-weight: bold">Exit Time</th>
            <th width="40px" style="text-align: left; font-size: 10px; font-weight: bold">Late</th>
            <th width="50px" style="text-align: center; font-size: 10px; font-weight: bold">Status</th>
            <th width="50px" style="text-align: center; font-size: 10px; font-weight: bold">Location</th>
        ...
        @foreach($dptname as $row)
            @if($spt->shift_id == $row->shift_id)
                ...
                <tr>
                    <td width="15px" style="border-bottom-width:1px; border-left-width:1px; font-size:8pt; text-align: left">{!! $sl !!}</td>
                    <td width="40px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->professional->pf_no !!}</td>                   
                    <td width="40px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->employee_id !!}</td>
                    ...
                    <td width="40px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->late_minute !!}</td>
                    
                    @if($row->leave_flag && $row->offday_flag == 1 || $row->leave_flag && $row->holiday_flag == 1)
                        <td width="50px" style="border-bottom-width:1px; font-size:8pt; text-align: center">{!! ($row->offday_flag == 1  ? '' : ($row->holiday_flag == 1 ? '': 'Absent')) !!}{!! $row->leave->short_code !!}</td>

                        @else
                        <td width="50px" style="border-bottom-width:1px;  font-size:8pt; text-align: center">{!! $row->attend_status == 'P' ? 'Present' : ($row->offday_flag == 1 ? 'Off Day' : ($row->holiday_flag == 1 ? 'Public Holiday' : ($row->leave_id  == 1  ? 'CL' : ($row->leave_id  == 2  ? 'SL' : ($row->leave_id  == 3  ? 'AL' :($row->leave_id  == 4  ? 'EL' :($row->leave_id  == 5  ? 'ML' :($row->leave_id  == 8  ? 'SPL':($row->leave_id  == 9 ? 'LWP':($row->leave_id  == 6  ? 'QL' :'Absent' )))))))))) !!}</td>
                        @endif
                        <td width="50px" style="border-bottom-width:1px; border-right-width:1px; font-size:8pt; text-align: left">{!! $row->location !!}</td>
                </tr>
                @php($sl++)
            @endif
        @endforeach
...
@endforeach
 ...
 <div class="blank-space"></div>
    ...
    <div class="blank-space"></div>
Total Manpower </span>: {!! $employees->count()+1 !!}</td>
