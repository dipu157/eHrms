...
<div>
    ...
    @if($data->contains('employee_id',$department[0]->headed_by))
        ...
        <tr class="row-line">
            ...
            <th width="70px" style="text-align: left; font-size: 10px; font-weight: bold">Joining Date</th>
        ...
        @foreach($data as $row)
             @if($row->employee_id == $department[0]->headed_by)
                ...
                <tr>
                    ...
                    <td width="110px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->professional->designation->name !!} </td>
                    <td width="70px" style="border-bottom-width:1px; border-right-width:1px; font-size:8pt; text-align: left">{!! \Carbon\Carbon::parse($row->joining_date)->format('d-M-Y') !!} </td>
...
@if($data->contains('shift_id',$spt->shift_id))
    ...
    <table class="table order-bank" width="90%" cellpadding="2">
        ...
        <tr class="row-line">
            ...
            <th width="110px" style="text-align: left; font-size: 10px; font-weight: bold">Name</th>
            <th width="110px" style="text-align: left; font-size: 10px; font-weight: bold">Designation</th>
            <th width="60px" style="text-align: left; font-size: 10px; font-weight: bold">Joining Date</th>
            <th width="50px" style="text-align: center; font-size: 10px; font-weight: bold">Status</th>
            <th width="90px" style="text-align: center; font-size: 10px; font-weight: bold">Location</th>
        ...
        @foreach($data as $row)
            @if($spt->shift_id == $row->shift_id)
                ...
                <tr>
                    ...
                    <td width="110px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->professional->personal->full_name !!} </td>
                    <td width="110px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->professional->designation->name !!} </td>
                    <td width="60px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! \Carbon\Carbon::parse($row->joining_date)->format('d-M-Y') !!} </td>
                    ...
                    @if($row->leave_flag && $row->offday_flag == 1 || $row->leave_flag && $row->holiday_flag == 1)
                        ...
                        <td width="90px" style="border-bottom-width:1px; border-right-width:1px;  font-size:8pt; text-align: left">{!! $row->location !!}</td>
