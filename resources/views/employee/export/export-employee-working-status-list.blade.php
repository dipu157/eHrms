        ...
        <tr>
            ...
            <td style="width:90%">
                <table style="width:100%" class="order-bank">
                    <thead>
                    <tr>
                        <td style="width:90%;" colspan="2"><span style="text-align:center; border: #000000; font-family:times;font-weight:bold;font-size:15pt;color:#000000; ">Employee List Status :{!! $status == null ? 'Existing New join' : $status->name !!}<br/>Total Manpower : {!! $employees->count() !!}</span></td>
                    </tr>
                    </thead>
                </table>
            </td>
    ...
    @if($employees->contains('department_id',$dept->department_id))
        ...
        <table class="table order-bank" width="90%" cellpadding="2">
            ...
            <tr class="row-line">
                ...
                <th width="27px" style="text-align: left; font-size: 10px; font-weight: bold">PF</th>
                ...
                @if($status != null)
                @if($status->id == 3 || $status->id == 4 || $status->id == 7 || $status->id == 9 || $status->id == 11)
                <th width="80px" style="text-align: right; font-size: 10px; font-weight: bold">{!! $status->name !!} Date</th>
                @endif
                @endif
                @if($status != null)
                @if($status->id == 2)
                <th width="70px" style="text-align: center; font-size: 10px; font-weight: bold">Confirmation<br/> Date</th>
                @endif
                @endif
            ...
            @foreach($employees as $row)            
                @if($dept->department_id == $row->department_id)
                    ...
                    <tr>
                        ...
                        <td width="27px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->pf_no !!}</td>
                        ...
                        <td width="100px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->designation->name !!}</td>
                        <td width="70px" style="border-bottom-width:1px; font-size:8pt; text-align: center">{!! \Carbon\Carbon::parse($row->joining_date)->format('d-M-Y') !!} </td>
             
                        @if($status != null)
                        @if($status->id == 3 || $status->id == 4 || $status->id == 7 || $status->id == 9 || $status->id == 11)
                        <td width="80px" style="border-bottom-width:1px; font-size:8pt; text-align: right">{!! $row->status_change_date !!}</td>
                        @endif
                        @endif
                        @if($status != null)
                        @if($status->id == 2)
                        <td width="70px" style="border-bottom-width:1px; font-size:8pt; text-align: right">{!! \Carbon\Carbon::parse($row->joining_date)->addMonth(6)->format('d-M-Y') !!}</td>
                        @endif
                        @endif
