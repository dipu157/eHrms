...
<body>
    ...
    <table border="0" cellpadding="0">
        ...
        <span style="text-align: center; font-size: 15px;"><br>
            ...
            <span style="text-align:center; border: #000000; font-family:times;font-weight:bold;font-size:18pt;color:#000000; ">Department :<u> {!! $designation == 'Doctor' ? 'Doctor' : ($designation == 'Nurse' ? 'Nurse' : ($designation == 'Consultant' ? 'Consultant' : ($designation == 'Other' ? 'Others' : ($designation == 'Cleaner' ? 'Cleaner' : ($designation == 'Pca' ? 'PCA' : ($designation == 'Admin' ? 'Admin' : 'All Designation' )))))) !!} </u></span>
        </span>
                    ...
                    <tbody>
                        ...
                        @foreach($employees as $row)
                                ...
                                <tr class="row-border">
                                            ...
                                            <td width="80px" style="font-size:10pt; text-align: left">{!! $row->pf_no ? $row->pf_no : " " !!} / {!! $row->employee_id ? $row->employee_id : " " !!}</td>
                                            <td width="120px" style="font-size:10pt; text-align: left">{!! $row->personal->full_name ? $row->personal->full_name : " " !!}</td>
                                            <td width="80px" style="font-size:10pt; text-align: left">{!! $row->personal->pm_district ? $row->personal->pm_district : " " !!}</td>
                                            <td width="110px" style="font-size:10pt; text-align: left">{!! $row->personal->last_education ? $row->personal->last_education : " " !!}</td>

                                            <td width="80px" style="font-size:10pt; text-align: left">{!! \Carbon\Carbon::parse($row->personal->dob ? $row->personal->dob : " ")->format('d-M-Y') !!}</td>

                                            <td width="70px" style="font-size:10pt; text-align: right">{!! \Carbon\Carbon::parse($row->joining_date ? $row->joining_date : " ")->format('d-M-Y') !!}</td>
                                            ...
                                            <td width="70px" style="font-size:10pt; text-align: right">{!! \Carbon\Carbon::parse($row->salary_properties->updated_at ? $row->salary_properties->updated_at : " ")->format('d-M-Y') !!}</td>
                                            <td width="125px" style="font-size:10pt; text-align: right">{!! $row->designation->name ? $row->designation->name : " " !!}</td>
