        ...
        <tr>
            ...
            @if(!empty($dept_data))
                <table style="width:100%" class="order-bank">
                    <thead>
                    <tr>
                        <td style="width:90%;" colspan="2"><span style="text-align:center; border: #000000; font-family:times;font-weight:bold;font-size:15pt;color:#000000; ">Overtime Report For  <br>{!! $dept_data->name !!}<br> OT Date  : {!! \Carbon\Carbon::parse($from_date)->format('d-M-Y') !!} To {!! \Carbon\Carbon::parse($to_date)->format('d-M-Y') !!}</span></td>
                    </tr>
    ...
    @if($data->contains('ot_date',$date))
        ...
        <table class="table order-bank" width="90%" cellpadding="2">
            ...
            <tr class="row-line">
                ...
                <th width="125px" style="text-align: left; font-size: 10px; font-weight: bold">Name</th>
                ...
                <th width="75px" style="text-align: left; font-size: 10px; font-weight: bold">Reason</th>
                <th width="60px" style="text-align: left; font-size: 10px; font-weight: bold">Status</th>
            </tr>
            </thead>
            <tbody>
                ...
                @foreach($data as $row)
                    @if($date == $row->ot_date)
                        <tr>
                            ...
                            <td width="125px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->professional->personal->full_name !!}<br/><span style="font-size:8pt; text-align: left;color: rebeccapurple">{!! $row->professional->department->name !!}</span></td>
                            ...
                            <td width="75px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->reason !!}</td>
                            <td width="60px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{{$row->approval_status==1 ? "Recommended" : "Apply"}}</td>
                        </tr>
